...
def get_columns(filters):
	party_type = 'Customer'#filters.get("party_type")
...
def get_party_addresses_and_contact(party_type, party, party_group):
	...
	for party, details in iteritems(party_details):
		...
		status_mitgliedschaft = frappe.get_doc("Customer", party).status_mitgliedschaft
		if not any([addresses, contacts]):
			result = [party]
			result.append(party_groups[party])
			result.append(status_mitgliedschaft)
